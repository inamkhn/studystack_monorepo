...
**[RESOLVED — fork/dashboard linkage was never wired]** Reusing `course_forks` (Feature 14) for per-student classroom progress only works if the teacher dashboard can actually join from a classroom roster to the right fork's `mastery_scores`/`quiz_attempts`. Add `classroom_students.fork_id` (FK → `course_forks.id`, set at invite-acceptance time when the student's fork is created). The dashboard aggregation query (step 3) now reads: `classroom_students` (for roster + `fork_id`) joined to `mastery_scores`/`quiz_attempts` via that `fork_id`, not via `course_id` directly — without this column there's no way to tell which fork belongs to which enrolled student.

**Data model:** `classrooms(id, teacher_id, course_id, consent_on_file, consent_document_url)`, `classroom_students(classroom_id, student_id, fork_id)` (new column), `users.age_bracket` (enum: `adult | minor_school_consented | unknown`); dashboard is a read/aggregate view over existing `mastery_scores`/`quiz_attempts`, joined through `classroom_students.fork_id` → `course_forks.id`. Per-student progress uses `course_forks` (Feature 14) rather than a separate fork table.
